...
public class UIOverlapHighlighter : MonoBehaviour
{
    ...
    [Range(0f, 1f)]
    public float requireCircleCoverage = 0.20f; // 20% overlap feels “touchy”
    public float paddingPixels = 0f; // + to shrink rects, - to grow

    [Header("Slider Related Attributes")]
    public BezierCurve curve;
    public VisualBPMManager visualBPMManager;
    ...
    {
        var cam =
            (_canvas && _canvas.renderMode == RenderMode.ScreenSpaceCamera)
                ? _canvas.worldCamera
                : null;

        float watchedPosition = visualBPMManager.GetWatchedPosition();

        hitZoneRT.position = curve.GetPoint(watchedPosition);
